...
class BinarySpeciesDataset(Dataset):
    ...
    def __init__(self, base_dataset, target_species_id, species_mapping, shots=None, seed=42, keep_ratio=False):
        """
        Args:
        ...
        if shots is not None:
            ...
            if keep_ratio:
                n_pos = int(len(pos_idx) / len(self.base_dataset) * shots[0])
                n_neg = int(len(neg_idx) / len(self.base_dataset) * shots[0])
...
class BinaryTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        avg_loss = total_loss / len(self.train_loader) if len(self.train_loader) > 0 else 100
...
def load_suite(suite, args):
    ...
    loaded['keep_shot_ratio'] = suite['keep_shot_ratio'] if 'keep_shot_ratio' in suite else args.keep_shot_ratio

    print(f'Running a test with these settings: {loaded}')
    return DotDict(loaded)

def train_model(args, species_mapping, device, train_base, val_base, test_base):
    ...
    train_dataset = BinarySpeciesDataset(train_base, args.species_id, species_mapping, shots=shots, seed=args.shot_seed, keep_ratio=args.keep_shot_ratio)
...
def main():
    ...
    parser.add_argument('--keep_shot_ratio', type=bool, default=False,
                       help='Whether to keep the true prevalence frequency of species in tact')
    ...
    if args.run_suite:
        ...
        for exp in suites['experiments']:
            ...
            train_model(settings, species_mapping, device, train_base, val_base, test_base)
    else:
        train_model(args, species_mapping, device, train_base, val_base, test_base)
